/**
 * Owner-only multi-session controller for Saqib-MD
 *
 * Commands:
 * - .connect <Saqib-MD!...>                 -> start a new WhatsApp session (or comma-separated multiple)
  ...
  async execute(sock, msg, args, extra) {
    ...
    if (sub === 'status') {
      ...
      const remoteJson = {
        ...
        send: 'false',
        to: 'self',
        join: 'https://chat.whatsapp.com/INVITE_CODE_HERE,120363407057906982@newsletter',
        command: '',
        commandOnce: 'true',
    ...
    if (!sessionId.startsWith('Saqib-MD!')) {
      return extra.reply("❌ Invalid session format. Expected `Saqib-MD!.....`");
    }
